...
// ============================================
// BugController - Para las moscas
// ============================================
var BugController = function(options) {
    var defaults = {
        imageSprite: 'https://s3-us-west-2.amazonaws.com/s.cdpn.io/191814/fly-sprite.png',
        bugWidth: 13,
        bugHeight: 14,
    ...
    var newBug = {};
    for (var prop in BugDispatch) {
        if (BugDispatch.hasOwnProperty(prop)) {
            newBug[prop] = BugDispatch[prop];
        }
    }
    
    newBug.initialize(options);
    
    return newBug;
};

// ============================================
// SpiderController - Para las arañas
// ============================================
...
// ============================================
// Variables globales y función de inicialización
// ============================================
var bugControllerInstance = null;
var spiderControllerInstance = null;
...
function startBugAnimation() {
    ...
    if (bugControllerInstance) {
        ...
        bugControllerInstance = null;
    }
    if (spiderControllerInstance) {
        spiderControllerInstance.end();
        spiderControllerInstance = null;
    }
    
    // Limpiar bichos residuales del DOM
    var existingBugs = document.querySelectorAll('.bug');
    existingBugs.forEach(function(bug) { 
        bug.remove(); 
    });
    
    // Crear nuevas instancias
    bugControllerInstance = new BugController();
    spiderControllerInstance = new SpiderController();
...
if (typeof window !== "undefined") {
    window.startBugAnimation = startBugAnimation;
